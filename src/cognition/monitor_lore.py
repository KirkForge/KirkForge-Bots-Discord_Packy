...
def main():
    ...
    p.add_argument("--checkpoints", type=int, default=3, help="how many checkpoints to average (default 3)")
    p.add_argument("--rel_imp_thresh", type=float, default=0.005, help="relative loss improvement threshold to stop (default 0.005 = 0.5%)")
    p.add_argument("--grad_norm_abort", type=float, default=20.0, help="absolute grad_norm abort threshold (default 20)")
    p.add_argument("--grad_rise_factor", type=float, default=1.5, help="if grad_norm increases by this factor over last N, abort (default 1.5)")
    p.add_argument("--hard_iter_cap", type=int, default=200, help="hard iteration cap fallback (default 200)")
    args = p.parse_args()
    ...
    if not pid:
        print("[monitor] No PID provided. You can still run monitor to watch logs and it will only print alerts (won't send signals).")
    else:
    ...
    for line in reader:
        ...
        if len(losses) >= args.checkpoints:
            ...
            print(f"[monitor] it={iters[-1] if iters else 'N/A'} loss={new_loss:.4f} rel_imp(last_window)={rel_imp:.4%} avg_grad={avg_grad:.3f}")
            ...
            if not abort and len(grads) >= args.checkpoints:
                if grads[0] > 0:
                    ...
                    if rise_factor >= args.grad_rise_factor:
                        abort = True
                        reason = f"grad_norm rose by factor {rise_factor:.2f} >= {args.grad_rise_factor}"

            # 3) loss plateau: relative improvement below threshold
            if not abort and rel_imp < args.rel_imp_thresh:
                abort = True
                reason = f"loss relative improvement {rel_imp:.4%} < threshold {args.rel_imp_thresh:.2%}"
            ...
            if abort:
                ...
                if pid:
                    ...
                    print("[monitor] No PID supplied - not signaling. Run with --pid or --procname to enable signaling.")
                break
